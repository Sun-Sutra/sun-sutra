...
import { TrendingUp, Settings, Link as LinkIcon } from 'lucide-react'
...
export default function BusinessModel() {
  ...
  return (
    <section id="model" style={{...sectionPad, background: 'var(--muted)', overflow: 'hidden'}}>
        ...
        <div ref={cardsRef} style={{
          ...
          {streams.map((s, index) => {
            ...
            onMouseLeave={e=>{e.currentTarget.style.transform='none'; e.currentTarget.style.boxShadow='var(--shadow-soft)'}}>
              <div className="pattern-vertical-lines-sm" style={{ position: 'absolute', inset: 0, opacity: 0.4, pointerEvents: 'none' }} />
              <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
                  <div style={{ width: 44, height: 44, borderRadius: '50%', background: 'var(--primary)', color: 'var(--primary-foreground)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                ...
                <h3 style={{fontFamily:'var(--ff-display)',fontSize:'1.4rem',fontWeight:700, color: 'var(--foreground)', lineHeight: 1.3, marginTop: '0.5rem'}}>{s.title}</h3>
      ...
      <style>{`
        @media(max-width:1024px){
          .bm-grid { grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)) !important; }
        }
      `}</style>
    </section>
  )
